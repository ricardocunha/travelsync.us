import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";

import { FormField } from "@/components/FormField";
import { SectionCard } from "@/components/SectionCard";
import { ParticipantDraftForm } from "@/features/plans/components/ParticipantDraftForm";
import type { ParticipantInput, PlanInput } from "@/features/plans/types";
import type { Country, Region } from "@/features/reference/types";
import { addParticipants, ApiError, createPlan, listCountries, listRegions } from "@/lib/api";
import { config } from "@/lib/config";

const wizardSteps = ["Basics", "Participants", "Regions", "Review"];

const timezoneOptions = [
  "America/Panama",
  "America/New_York",
  "America/Sao_Paulo",
  "Europe/Madrid",
  "UTC",
];

const now = new Date("2026-04-06T14:00:00.000Z");
const later = new Date("2026-04-10T20:00:00.000Z");

function toLocalSeed(date: Date) {
  return date.toISOString().slice(0, 16);
}

export function NewPlanPage() {
  const navigate = useNavigate();
  const [stepIndex, setStepIndex] = useState(0);
  const [regions, setRegions] = useState<Region[]>([]);
  const [countries, setCountries] = useState<Country[]>([]);
  const [participants, setParticipants] = useState<ParticipantInput[]>([]);
  const [message, setMessage] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);

  const [draft, setDraft] = useState({
    organization_id: config.organizationId,
    created_by_user_id: 1,
    name: "",
    description: "",
    event_start: toLocalSeed(now),
    event_end: toLocalSeed(later),
    event_timezone: "America/Panama",
    arrival_buffer_hours: 12,
    departure_buffer_hours: 4,
    max_budget_per_person: "1200",
    currency: "USD",
    cabin_class: "ECONOMY",
    search_mode: "strict",
    region_filter_ids: [] as number[],
  });

  useEffect(() => {
    void Promise.all([listRegions(), listCountries()])
      .then(([regionItems, countryItems]) => {
        setRegions(regionItems);
        setCountries(countryItems);
      })
      .catch(() => {
        setMessage(
          `Reference data could not be loaded from the Go API at ${config.apiBaseUrl}. ` +
            "Check that apps/api is running and that WEB_ALLOWED_ORIGINS includes your Vite origin, " +
            "or set VITE_ENABLE_MOCK=true for frontend-only mode.",
        );
      });
  }, []);

  function updateDraft(key: string, value: string | number | number[]) {
    setDraft((current) => ({
      ...current,
      [key]: value,
    }));
  }

  function toggleRegion(regionId: number) {
    setDraft((current) => ({
      ...current,
      region_filter_ids: current.region_filter_ids.includes(regionId)
        ? current.region_filter_ids.filter((value) => value !== regionId)
        : [...current.region_filter_ids, regionId],
    }));
  }

  function nextStep() {
    setStepIndex((current) => Math.min(current + 1, wizardSteps.length - 1));
  }

  function previousStep() {
    setStepIndex((current) => Math.max(current - 1, 0));
  }

  function appendParticipant(participant: ParticipantInput) {
    setParticipants((current) => [...current, participant]);
  }

  async function handleSave() {
    if (!draft.name.trim()) {
      setMessage("Plan name is required before saving.");
      return;
    }

    if (new Date(draft.event_end).getTime() <= new Date(draft.event_start).getTime()) {
      setMessage("Event end must be after event start.");
      return;
    }

    setSaving(true);
    setMessage(null);

    const planInput: PlanInput = {
      organization_id: draft.organization_id,
      created_by_user_id: draft.created_by_user_id,
      name: draft.name.trim(),
      description: draft.description.trim(),
      event_start: new Date(draft.event_start).toISOString(),
      event_end: new Date(draft.event_end).toISOString(),
      event_timezone: draft.event_timezone,
      arrival_buffer_hours: Number(draft.arrival_buffer_hours),
      departure_buffer_hours: Number(draft.departure_buffer_hours),
      max_budget_per_person: draft.max_budget_per_person ? Number(draft.max_budget_per_person) : null,
      currency: draft.currency,
      cabin_class: draft.cabin_class,
      search_mode: draft.search_mode,
      region_filter_ids: draft.region_filter_ids,
    };

    try {
      const createdPlan = await createPlan(planInput);
      if (participants.length > 0) {
        await addParticipants(createdPlan.id, participants);
      }
      navigate(`/plans/${createdPlan.id}`);
    } catch (error) {
      setMessage(error instanceof ApiError ? error.message : "Unable to save the plan.");
    } finally {
      setSaving(false);
    }
  }

  return (
    <div className="space-y-6">
      {/* Progress steps */}
      <div className="flex items-center gap-1.5 sm:gap-2">
        {wizardSteps.map((step, index) => (
          <div
            className="flex items-center gap-1.5 sm:gap-2"
            key={step}
          >
            <button
              className={`flex h-8 w-8 items-center justify-center rounded-lg text-xs font-semibold transition ${
                index === stepIndex
                  ? "bg-[var(--accent)] text-white shadow-[0_0_12px_rgba(99,102,241,0.3)]"
                  : index < stepIndex
                    ? "bg-[var(--accent-muted)] text-[color:var(--accent-hover)]"
                    : "bg-[var(--bg-overlay)] text-[color:var(--text-tertiary)]"
              }`}
              onClick={() => setStepIndex(index)}
              type="button"
            >
              {index + 1}
            </button>
            <span
              className={`hidden text-sm font-medium sm:inline ${
                index === stepIndex ? "text-[color:var(--text-primary)]" : "text-[color:var(--text-tertiary)]"
              }`}
            >
              {step}
            </span>
            {index < wizardSteps.length - 1 ? (
              <div className={`h-px w-6 sm:w-10 ${index < stepIndex ? "bg-[var(--accent)]" : "bg-[var(--border-default)]"}`} />
            ) : null}
          </div>
        ))}
      </div>

      {/* Step 1: Basics */}
      {stepIndex === 0 ? (
        <SectionCard className="p-6 sm:p-8">
          <p className="label-mono text-[color:var(--accent)]">Step 1</p>
          <h2 className="heading-section mt-2 text-3xl">Plan basics & guardrails</h2>
          <p className="mt-2 text-sm text-[color:var(--text-secondary)]">
            Capture the constraints first, shape the departure map second.
          </p>

          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <FormField label="Plan name">
              <div className="input-wrap px-3 py-2.5">
                <input
                  className="w-full bg-transparent text-sm outline-none placeholder:text-[color:var(--text-tertiary)]"
                  onChange={(event) => updateDraft("name", event.target.value)}
                  placeholder="EMEA + Americas leadership sync"
                  value={draft.name}
                />
              </div>
            </FormField>

            <FormField label="Event timezone">
              <div className="input-wrap px-3 py-2.5">
                <select
                  className="w-full bg-transparent text-sm outline-none"
                  onChange={(event) => updateDraft("event_timezone", event.target.value)}
                  value={draft.event_timezone}
                >
                  {timezoneOptions.map((timezone) => (
                    <option
                      key={timezone}
                      value={timezone}
                    >
                      {timezone}
                    </option>
                  ))}
                </select>
              </div>
            </FormField>

            <FormField label="Event start">
              <div className="input-wrap px-3 py-2.5">
                <input
                  className="w-full bg-transparent text-sm outline-none"
                  onChange={(event) => updateDraft("event_start", event.target.value)}
                  type="datetime-local"
                  value={draft.event_start}
                />
              </div>
            </FormField>

            <FormField label="Event end">
              <div className="input-wrap px-3 py-2.5">
                <input
                  className="w-full bg-transparent text-sm outline-none"
                  onChange={(event) => updateDraft("event_end", event.target.value)}
                  type="datetime-local"
                  value={draft.event_end}
                />
              </div>
            </FormField>

            <FormField label="Arrival buffer (hours)">
              <div className="input-wrap px-3 py-2.5">
                <input
                  className="w-full bg-transparent text-sm outline-none"
                  min={0}
                  onChange={(event) => updateDraft("arrival_buffer_hours", Number(event.target.value))}
                  type="number"
                  value={draft.arrival_buffer_hours}
                />
              </div>
            </FormField>

            <FormField label="Departure buffer (hours)">
              <div className="input-wrap px-3 py-2.5">
                <input
                  className="w-full bg-transparent text-sm outline-none"
                  min={0}
                  onChange={(event) => updateDraft("departure_buffer_hours", Number(event.target.value))}
                  type="number"
                  value={draft.departure_buffer_hours}
                />
              </div>
            </FormField>

            <FormField label="Budget per traveler">
              <div className="input-wrap px-3 py-2.5">
                <input
                  className="w-full bg-transparent text-sm outline-none"
                  min={0}
                  onChange={(event) => updateDraft("max_budget_per_person", event.target.value)}
                  placeholder="1200"
                  type="number"
                  value={draft.max_budget_per_person}
                />
              </div>
            </FormField>

            <FormField label="Currency">
              <div className="input-wrap px-3 py-2.5">
                <input
                  className="w-full bg-transparent text-sm outline-none"
                  maxLength={3}
                  onChange={(event) => updateDraft("currency", event.target.value.toUpperCase())}
                  value={draft.currency}
                />
              </div>
            </FormField>

            <FormField label="Cabin class">
              <div className="input-wrap px-3 py-2.5">
                <select
                  className="w-full bg-transparent text-sm outline-none"
                  onChange={(event) => updateDraft("cabin_class", event.target.value)}
                  value={draft.cabin_class}
                >
                  <option value="ECONOMY">Economy</option>
                  <option value="PREMIUM_ECONOMY">Premium economy</option>
                  <option value="BUSINESS">Business</option>
                  <option value="FIRST">First</option>
                </select>
              </div>
            </FormField>

            <FormField label="Search mode">
              <div className="input-wrap px-3 py-2.5">
                <select
                  className="w-full bg-transparent text-sm outline-none"
                  onChange={(event) => updateDraft("search_mode", event.target.value)}
                  value={draft.search_mode}
                >
                  <option value="strict">Strict</option>
                  <option value="flexible">Flexible</option>
                </select>
              </div>
            </FormField>
          </div>

          <div className="mt-5">
            <FormField
              hint="Optional"
              label="Description"
            >
              <div className="input-wrap px-3 py-2.5">
                <textarea
                  className="min-h-28 w-full resize-y bg-transparent text-sm outline-none placeholder:text-[color:var(--text-tertiary)]"
                  onChange={(event) => updateDraft("description", event.target.value)}
                  placeholder="Who is meeting, why this event matters, and any planning context..."
                  value={draft.description}
                />
              </div>
            </FormField>
          </div>
        </SectionCard>
      ) : null}

      {/* Step 2: Participants */}
      {stepIndex === 1 ? (
        <SectionCard className="p-6 sm:p-8">
          <p className="label-mono text-[color:var(--accent)]">Step 2</p>
          <h2 className="heading-section mt-2 text-3xl">Capture the departure map</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[color:var(--text-secondary)]">
            Add guest participants with per-person airport choices. You can always add more travelers later from the plan detail page.
          </p>

          <div className="mt-6">
            <ParticipantDraftForm
              countries={countries}
              onAdd={appendParticipant}
            />
          </div>

          <div className="mt-6 space-y-2.5">
            {participants.length === 0 ? (
              <div className="chip px-4 py-4 text-sm text-[color:var(--text-secondary)]">
                No participants staged yet. Saving without participants is allowed, but search orchestration later will require at least one.
              </div>
            ) : (
              participants.map((participant, index) => (
                <div
                  className="chip flex flex-wrap items-center justify-between gap-3 px-4 py-3.5"
                  key={`${participant.guest_name}-${index}`}
                >
                  <div>
                    <div className="text-sm font-semibold">{participant.guest_name}</div>
                    <div className="mt-0.5 text-xs text-[color:var(--text-tertiary)]">
                      {participant.departure_city} · airport #{participant.departure_airport_id}
                    </div>
                  </div>
                  <button
                    className="text-xs font-medium text-[color:var(--rose)] transition hover:text-[color:var(--text-primary)]"
                    onClick={() =>
                      setParticipants((current) =>
                        current.filter((_, participantIndex) => participantIndex !== index),
                      )
                    }
                    type="button"
                  >
                    Remove
                  </button>
                </div>
              ))
            )}
          </div>
        </SectionCard>
      ) : null}

      {/* Step 3: Regions */}
      {stepIndex === 2 ? (
        <SectionCard className="p-6 sm:p-8">
          <p className="label-mono text-[color:var(--accent)]">Step 3</p>
          <h2 className="heading-section mt-2 text-3xl">Optional region filter</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[color:var(--text-secondary)]">
            Region filtering helps the search stay focused when you already know the broad zone worth exploring.
          </p>

          <div className="mt-6 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {regions.map((region) => {
              const checked = draft.region_filter_ids.includes(region.id);

              return (
                <button
                  className={`rounded-xl border px-5 py-4 text-left transition ${
                    checked
                      ? "border-[var(--accent)] bg-[var(--accent-muted)]"
                      : "border-[var(--border-default)] bg-[var(--bg-overlay)] hover:border-[var(--border-strong)]"
                  }`}
                  key={region.id}
                  onClick={() => toggleRegion(region.id)}
                  type="button"
                >
                  <div className={`label-mono text-[0.6rem] ${checked ? "text-[color:var(--accent-hover)]" : ""}`}>
                    Region
                  </div>
                  <div className="heading-section mt-2 text-xl">{region.name}</div>
                </button>
              );
            })}
          </div>
        </SectionCard>
      ) : null}

      {/* Step 4: Review */}
      {stepIndex === 3 ? (
        <SectionCard className="p-6 sm:p-8">
          <p className="label-mono text-[color:var(--accent)]">Step 4</p>
          <h2 className="heading-section mt-2 text-3xl">Review & create</h2>

          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            <div className="card-dark rounded-xl p-5">
              <div className="label-mono text-[color:var(--accent)]">Event setup</div>
              <div className="mt-4 space-y-2.5 text-sm leading-7 text-[color:var(--text-secondary)]">
                <p><strong className="text-[color:var(--text-primary)]">Name:</strong> {draft.name || "Untitled plan"}</p>
                <p><strong className="text-[color:var(--text-primary)]">Timezone:</strong> {draft.event_timezone}</p>
                <p><strong className="text-[color:var(--text-primary)]">Search mode:</strong> {draft.search_mode}</p>
                <p><strong className="text-[color:var(--text-primary)]">Cabin class:</strong> {draft.cabin_class}</p>
                <p><strong className="text-[color:var(--text-primary)]">Budget:</strong> {draft.max_budget_per_person ? `${draft.currency} ${draft.max_budget_per_person}` : "Open"}</p>
              </div>
            </div>
            <div className="chip p-5">
              <div className="label-mono">Team scope</div>
              <div className="mt-4 space-y-2.5 text-sm leading-7 text-[color:var(--text-secondary)]">
                <p><strong className="text-[color:var(--text-primary)]">Participants staged:</strong> {participants.length}</p>
                <p><strong className="text-[color:var(--text-primary)]">Region filters:</strong> {draft.region_filter_ids.length === 0 ? "All regions" : draft.region_filter_ids.length}</p>
                <p><strong className="text-[color:var(--text-primary)]">Organization:</strong> {draft.organization_id}</p>
                <p><strong className="text-[color:var(--text-primary)]">Created by user:</strong> {draft.created_by_user_id}</p>
              </div>
            </div>
          </div>
        </SectionCard>
      ) : null}

      {message ? <p className="text-sm font-medium text-[color:var(--rose)]">{message}</p> : null}

      <div className="flex items-center justify-between gap-3">
        <button
          className="btn-secondary"
          disabled={stepIndex === 0}
          onClick={previousStep}
          type="button"
        >
          Back
        </button>

        {stepIndex < wizardSteps.length - 1 ? (
          <button
            className="btn-primary"
            onClick={nextStep}
            type="button"
          >
            Continue
          </button>
        ) : (
          <button
            className="btn-primary"
            disabled={saving}
            onClick={() => void handleSave()}
            type="button"
          >
            {saving ? "Creating..." : "Create Plan"}
          </button>
        )}
      </div>
    </div>
  );
}
