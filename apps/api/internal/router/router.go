...
func New(api *handler.API, allowedOrigins []string) http.Handler {
	...
	mux.HandleFunc("POST /api/v1/plans/{id}/search", api.StartSearch)
	mux.HandleFunc("GET /api/v1/plans/{id}/search/status", api.GetSearchStatus)
	mux.HandleFunc("GET /api/v1/plans/{id}/destinations", api.ListDestinationResults)
	mux.HandleFunc("GET /api/v1/plans/{id}/destinations/{did}", api.GetDestinationResult)
